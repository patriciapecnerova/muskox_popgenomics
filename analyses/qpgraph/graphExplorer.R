...
# run nruns optimizations and save best graph for each run and fit it
for(i in 1:pars$nruns){
    
    optimres <- find_graphs(f2s, numadmix=pars$nadmix, outpop=pars$outpop)
    winners[[i]] <- optimres[which.min(optimres$score),]
    fits[[i]] <- qpgraph(f2s, winners[[i]]$graph[[1]], return_fstats=T)
